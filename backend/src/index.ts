...
function removePlayerFromLobby(lobby: Lobby, targetPlayerId: string): RemovePlayerResult {
...
app.post<{
	Params: { code: string };
	Body: { actorId?: string; targetPlayerId?: string };
}>('/api/lobbies/:code/player/remove', async (request, reply) => {
	const code = request.params.code.toUpperCase();
	const lobby = lobbies.get(code);
	if (!lobby) return reply.code(404).send({ error: 'Lobby not found' });
	...
	if (!actor) return reply.code(403).send({ error: 'Actor not in lobby' });
